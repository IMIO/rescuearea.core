...
class IKeysCodeAccessBadgeFieldsRowSchema(model.Schema):
    ...
    last_key_check_date = schema.Date(title=_(u"Last Key Check Date"), required=False)
...
class IHistoryRowSchema(model.Schema):
    modified_field = schema.TextLine(title=_(u"Modified field"), required=False)
...
class IPpi(model.Schema):
    ...
    address = ObjectField(title=_(u"Address"), schema=IAddressRowSchema, required=True)
    ...
    type_of_activity = schema.TextLine(
        ...
        description=_(
            u"<p>Factory, warehouse, store, MRPA... Public access, number of people, valid? autonomous? sleeping? Describe in a few lines what it is about.</p>"
        ),
        required=False,
    )

    concierge_service = schema.Bool(title=_(u"Concierge Service?"), required=False)
    ...
    existence_of_a_ppui = schema.Text(
        ...
        description=_(
            u"<p>If yes, briefly describe the multidisciplinary principles...</p>"
        ),
        required=False,
    )
    ...
    adaptation_of_the_emergency_services_in_relation_to_the_response_plan = RichText(
        title=_(
            u"Adaptation of the emergency services in relation to the response plan"
        ),
        description=_(
            u"<p>How should the dispatch of help be adapted for this site in relation to the usual procedures?</p>"
        ),
        required=False,
    )
    ...
    specific_equipment_to_take_with_you = RichText(
        ...
        description=_(
            u"<p>For example UHF relay for buildings with insufficient UHF range</p>"
        ),
        required=False,
    )
    ...
    route_to_follow = RichText(
        ...
        description=_(
            u"<p>Is it always the same, or according to the intervention, you must take into account the direction of the wind, or arrive by a particular access to the site according to the place where the event occurred</p>"
        ),
        required=False,
    )
    ...
    site_particularities = RichText(
        ...
        description=_(
            u"<p>Describe in a few words why this site is special and has been the subject of a PPI.</p><p>Example: Access not very obvious, complex architecture, exceptional risk(s) present, means of control available on site for firefighters, place for which an intervention will be arduous because of the absence of compartmentalization...</p>"
        ),
        required=False,
    )
    ...
    instructions_for_the_operator = RichText(
        ...
        description=_(
            u"<p>Specify here if specific actions are to be taken by the operator.</p><p>For example, if the call comes from a private individual, it is imperative to contact the lodge on duty to inform those in charge of the site of our arrival, and thus be welcomed.</p>"
        ),
        required=False,
    )
    ...
    fieldset(
        ...
        fields=["vehicle_stop_emergency_reception_point", "point_of_first_destination"],
    )

    vehicle_stop_emergency_reception_point = RichText(
        title=_(u"Vehicle stop/emergency reception point"),
        description=_(
            u"<p>Specify here if a meeting point is planned with site managers for reasons of access complexity, or if depending on a chemical scenario you should not get too close...</p>"
        ),
        required=False,
    )
    ...
    point_of_first_destination = RichText(
        ...
        description=_(
            u"<p>Specify if a particular place has been designed to gather emergency services in the event of an increase in power. Important mainly if reflex reinforcements are sent, or in case of chemical intervention, for example.</p>"
        ),
        required=False,
    )
    ...
    special_means_of_protection_to_wear = RichText(
        ...
        description=_(
            u"<p>Specify here the means that must be taken in addition to firefighting, or the personal protective equipment imposed on an industrial site for example</p>"
        ),
        required=False,
    )
    ...
    materials_equipment_available = RichText(
        ...
        description=_(
            u"<p>Identify the main risks of the site. limit yourself to about 5 maximum.</p><p>Completeness = drown the fish!</p>"
        ),
        required=False,
    )
    ...
    risks_present = RichText(
        ...
        description=_(
            u"<p>we need to know what's available to assist us in intervention. It is not necessary to mention the presence of fire extinguishers or even reels intended for the PPE of the site. On the other hand, hydrants, dry or wet columns... must be mentioned.</p>"
        ),
        required=False,
    )
    ...
    deadline_for_searching_for_additional_information = schema.Date(
        title=_(u"Deadline for searching for additional information"), required=False
    )
    ...
    availability_of_paper_copies = schema.Text(
        ...
        description=_(
            u"<p>Specify the places where it is necessary to have paper copies available </p><ul><li>DZHC</li><li>Fastest first aid station (several if the means must come from several stations)</li></ul>"
        ),
        required=False,
    )
...
class PpiView(view.DefaultView):
    ...
    def getNumStreet(self):
        if getattr(self.context.address, "number", None):
            return "{0} {1}".format(
                self.context.address.number.encode("utf8"),
                self.context.address.street.encode("utf8"),
            )
        return self.context.address.street

    def getZipTown(self):
        return "{0} {1}".format(
            self.context.address.zip_code, self.context.address.commune.encode("utf8")
        )

    def getCoord(self):
        if getattr(self.context.address, "longitude", None) or getattr(
            self.context.address, "latitude", None
        ):
            return "{0}/{1}".format(
    ...
    def itinerary(self):
        if getattr(self.context, "route_to_follow", None) and getattr(
            self.context, "appendix_itinerary", None
        ):
            return True
...
class IconsView(BrowserView):
    def __call__(self):
        field_with_icon = [
            ["keys_code_access_badge", "existence_keys_code_access_badge"]
        ]
            ...
            if type(field) == list:
                if getattr(getattr(self.context, field[0]), field[1]):
                    ...
                    if icon:
                        html = img.format(
                            html,
                            "{0}/{1}".format(api.portal.get().absolute_url(), icon),
                        )
            else:
...
def zip_code_town(object):
    ...
    if address:
        return u"{0} {1}".format(
            getattr(address, "zip_code", None), getattr(address, "commune", None)
        )
...
class ImplementationPlanValidator(validator.SimpleFieldValidator):
    def validate(self, value):
        ...
        if not self.request.form.get(dependency_key) and not value:
            raise Invalid(
                _(
                    u"The implementation plan is mandatory if the ppi data is "
...
validator.WidgetValidatorDiscriminators(
    ImplementationPlanValidator, field=IPpi["appendix_implementation_plan"]
)
